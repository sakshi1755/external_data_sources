"""
Load JoSAA clean parquet from GCS into BigQuery.

Each table in sources.py is loaded via load_table_from_uri with
WRITE_TRUNCATE — fully replaces the destination on every run. Idempotent.

Pre-reqs:
  - dataset avantifellows.external_data_sources exists (asia-south1)
  - clean parquet staged in gs://avantifellows-external-data/josaa/clean/
    (run build_clean.py + upload_to_gcs.py first)

Usage:
  python3 scripts/load_bq.py                          # load all tables
  python3 scripts/load_bq.py --table josaa_fact_cutoffs
  python3 scripts/load_bq.py --dry-run                # show plan
"""
...
def main() -> None:
    ...
    ap.add_argument("--table", default=None, help="Load only this BQ table name")
    ...
    print(f"JoSAA → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
